...
describe('validateSchedule', () => {
  ...
  it('detects wrong number of players on field', () => {
    const [p1, p2] = [playerFactory.build({ name: 'A' }), playerFactory.build({ name: 'B' })];
    const rotations: Rotation[] = [
      buildRotation(0, {
        [p1.id]: RotationAssignment.Goalie,
        [p2.id]: RotationAssignment.Field,
...
describe('validateRosterForGame', () => {
  it('returns error when not enough players for the field size', () => {
    ...
    expect(errors).toContainEqual(expect.stringContaining('Not enough players'));
  });
  ...
  it('warns when all players must be on field (no subs possible)', () => {
    ...
    expect(errors).toContainEqual(expect.stringContaining('no substitutions possible'));
  });
  ...
  it('returns error when no goalie-eligible players', () => {
    ...
    expect(errors).toContainEqual(expect.stringContaining('goalie-eligible'));
  });
  ...
  it('returns error when not enough goalies for goalie rest rotation', () => {
    ...
    expect(errors).toContainEqual(expect.stringContaining('Need at least 2 goalie-eligible'));
  });
